...
const links = [
  ...
  "https://helda.helsinki.fi/handle/10138/601985",
  "https://helda.helsinki.fi/handle/10138/592584",
  "https://aaltodoc.aalto.fi/handle/123456789/120907",
];

async function run() {
  for (const link of links) {
    const result = await analyzeThesisLink(link);

    console.log(`Thesis URL: ${result.thesisUrl}`);
    console.log(`Decision: ${result.decision}`);
    console.log(`Evidence: ${result.evidence}`);
    console.log(`Decision source: ${result.decisionSource}`);

    if (result.pdfLinksChecked.length) {
      console.log(`PDF links checked: ${result.pdfLinksChecked.join(", ")}`);
    }
